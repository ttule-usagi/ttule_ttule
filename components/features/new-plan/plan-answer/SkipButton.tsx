...
export default function SkipButton({ label = '나중에 할게요.', onClick }: Props) {
  return (
    <div className='flex flex-col items-end gap-1 self-end'>
      <div className='relative'>
        <div className='bg-gradient-to-b from-brand-blue-500 to-brand-blue-400 rounded-lg p-4'>
          <button
            type='button'
            onClick={onClick}
            className='bg-white rounded-lg px-3 py-2 text-typo-description text-brand-blue-700 font-medium whitespace-nowrap'
          >
            {label}
          </button>
        </div>
      ...
      <p className='text-typo-caption text-brand-blue-400 pr-1'>클릭하고 넘기기</p>
    </div>
  );
}
