...
- 날짜별 여행 일정을 편집하고 이동 동선(예정)을 지도에서 확인
- 단일 뷰와 전체 뷰를 전환하며 일정 관리
- 장소 리스트에서 원하는 날짜로 드래그하여 일정 추가 및 수정
- 장소 일정과 메모 일정을 함께 작성하여 자유로운 계획 구성
- 계획 정보와 공유 설정 관리
- 장소 컴포넌트간 Google Routes API 기반 대중교통 이동 시간 제공, 모달에서 Google Maps 실제 경로 보기 연결
- 장소 이동 메모 - 이동수단 및 메모 관리
