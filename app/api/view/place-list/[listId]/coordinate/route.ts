import { getPlaceListPlacesCoordinate } from '@/lib/actions/api/placeList';
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ listId: string }> }) {
  ...
  try {
    const data = await getPlaceListPlacesCoordinate({ supabase, listId });
    return NextResponse.json(data);
  } catch (error: unknown) {
    console.error('❌ 저장된 장소 마커 조회 실패: ', error);
